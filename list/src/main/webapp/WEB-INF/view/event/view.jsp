	...
	<div id="container">
		...
		<div id="outbox">		
			<div id="left">
				...
				<ul>	
					<li><a href="list" id="leftNavi1">이벤트 게시판</a></li>
			...
			<div id="contents">
				<div id="customer">
					<h2><strong>이벤트 게시판</strong><span>${map.boardVo1.bid} : 쟈뎅샵 소식을 전해드립니다.</span></h2>

					<div class="viewDivMt">
						<div class="viewHead">
							<div class="subject">
								<ul>
									<li>${map.boardVo1.btitle}</li>
								</ul>
							</div>
							<div class="day">
								<p class="txt">작성일<span>${map.boardVo1.bdate}</span></p>
							</div>
						</div>

						<div class="viewContents">
							${map.boardVo1.bcontent }
						</div>
					</div>
				...
				<script type="text/javascript">
                  function replybtn() {
                     ...
                     $.ajax({
                        ...
                        success:function(data){ //EventVo,replyCount
                           alert("성공")
                        var html ="";
                        var count = data.replyCount
                        $(".orange").text(count);
                        if(data != null){
                        html += '<ul id="'+data.EventVo.cno+'">';
                        html += '<li class="name">'+data.EventVo.id+'<span>['+data.EventVo.cdate+']</span></li>';
                        html += '<li class="txt">'+data.EventVo.ccontent+'</li>';
                           html += '<li class="btn">';
					...
					<input type="text" id="bid" value="${param.bid}">
					<input type="text" id="id" value="bbb">
						<ul>
							<li class="in">
								<p class="txt">총 <span class="orange">${map.count}</span> 개의 댓글이 달려있습니다.</p>
								<p class="password">비밀번호&nbsp;&nbsp;<input type="password" class="replynum" /></p>
								<textarea class="replyType" id="ccontent"></textarea>
							</li>
							<li class="btn">
							<a class="replyBtn" onclick="replybtn()" id="">등록</a>
							</li>
						</ul>
					...
					<c:forEach items="${map.replylist }" var="reply">
						<ul id="${reply.cno}">
							<li class="name">${reply.id }<span>[${reply.cdate }]</span></li>
							<li class="txt">${reply.ccontent }</li>
							<c:if test="${reply.id  == '58'}">
							<li class="btn">
								<a href="#" class="rebtn">수정</a>
								<a href="#" class="rebtn">삭제</a>
							</li>
